...
import "./libraries/AqueductV1Library.sol";
...
contract AqueductV1Router is IAqueductV1Router {
        ...
        returns (uint256 amountToken, uint256 amountETH, uint256 liquidity)
    {
        ...
        address pair = AqueductV1Library.pairFor(factory, token, WETH);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = AqueductV1Library.sortTokens(input, output);
            IAqueductV1Pair pair = IAqueductV1Pair(AqueductV1Library.pairFor(factory, input, output));
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = AqueductV1Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) = input == token0
