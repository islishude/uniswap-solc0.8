...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20, SuperAppBase {
    ...
    function _getReservesAtTimeNoFees(
        ...
        if (totalFlow0 > 0 && totalFlow1 > 0 && timeElapsed > 0) {
            // use approximation:
            uint112 totalAmount0 = (totalFlow0 *
                timeElapsed *
                (10000 - UPPER_BOUND_FEE)) / 10000; // apply upper bound fee to input amounts
            uint112 totalAmount1 = (totalFlow1 *
                timeElapsed *
                (10000 - UPPER_BOUND_FEE)) / 10000;
            // not sure if these uint256->uint112 downcasts are safe:
            _reserve0 = uint112(
                Math.sqrt(
                    (_kLast * (reserve0 + totalAmount0)) /
    ...
    function _getReservesAtTime(
        ...
        uint32 timeElapsed = time - blockTimestampLast;
        (_reserve0, _reserve1) = _getReservesAtTimeNoFees(
            time,
            totalFlow0,
            totalFlow1
        );
        ...
        if (totalFlow0 > 0 && totalFlow1 > 0) {
            ...
            if (flowDirection > totalFlow1) {
                ...
                _reserve0 += uint112(
                    uint(
                        int(
                            uint(totalFlow0 * timeElapsed * LOWER_BOUND_FEE) /
                                10000
                        ) +
                            (reserve0Diff -
                                ((reserve0Diff *
                                    int112(10000 - LOWER_BOUND_FEE)) /
                                    int112(10000 - UPPER_BOUND_FEE)))
                    )
                );
                _reserve1 += ((totalFlow1 * timeElapsed * LOWER_BOUND_FEE) /
                    10000);
            } else if (flowDirection < totalFlow1) {
                int reserve1Diff = (int(uint(reserve1)) - int(uint(_reserve1)));
                _reserve0 +=
                    ((totalFlow0 * timeElapsed * LOWER_BOUND_FEE) / 10000) +
                    reserve0 -
                    _reserve0;
                _reserve1 += uint112(
                    uint(
                        int(
                            uint(totalFlow1 * timeElapsed * LOWER_BOUND_FEE) /
                                10000
                        ) +
                            (reserve1Diff -
                                ((reserve1Diff *
                ...
                _reserve0 +=
                    (totalFlow0 * timeElapsed * LOWER_BOUND_FEE) /
                    10000;
                _reserve1 +=
                    (totalFlow1 * timeElapsed * LOWER_BOUND_FEE) /
                    10000;
            }
    ...
    function getReservesAtTime(
        uint32 time
    ) public view returns (uint112 _reserve0, uint112 _reserve1) {
        uint112 totalFlow0 = uint112(
            uint96(cfa.getNetFlow(token0, address(this)))
        );
        uint112 totalFlow1 = uint112(
            uint96(cfa.getNetFlow(token1, address(this)))
        );
        (_reserve0, _reserve1) = _getReservesAtTime(
            time,
            totalFlow0,
            totalFlow1
        );
    ...
    function getRealTimeReserves()
        public
        view
        returns (uint112 _reserve0, uint112 _reserve1, uint time)
    {
        (_reserve0, _reserve1) = getReservesAtTime(
    ...
    function _getUpdatedCumulatives(
        ...
        internal
        view
        returns (uint _twap0CumulativeLast, uint _twap1CumulativeLast)
    {
        uint32 timeElapsed = time - blockTimestampLast;
        ...
        if (totalFlow0 > 0 && totalFlow1 > 0) {
            ...
            _twap1CumulativeLast += uint256(
                UQ112x112.encode(reserve1 - _reserve1).uqdiv(totalFlow0)
            );
        } else if (totalFlow1 > 0) {
            _twap0CumulativeLast += uint256(
                UQ112x112.encode(reserve0 - _reserve0).uqdiv(totalFlow1)
            );
        }
    }
    ...
    function _getUserBalancesAtTime(
        ...
        (
            uint _twap0CumulativeLast,
            uint _twap1CumulativeLast
        ) = _getUpdatedCumulatives(
                time,
                _reserve0,
                _reserve1,
                totalFlow0,
        ...
        balance0 = UQ112x112.decode(
            uint256(uint96(flow1)) *
                (_twap0CumulativeLast - userStartingCumulatives0[user])
        ); // flow in is always positive
        balance1 = UQ112x112.decode(
            uint256(uint96(flow0)) *
                (_twap1CumulativeLast - userStartingCumulatives1[user])
        );
    }
    ...
    function getUserBalancesAtTime(
        ...
        (uint112 _reserve0, uint112 _reserve1) = _getReservesAtTimeNoFees(
            time,
            totalFlow0,
            totalFlow1
        );
    ...
    function getRealTimeUserBalances(
        address user
    ) public view returns (uint balance0, uint balance1, uint time) {
        (balance0, balance1) = getUserBalancesAtTime(
            user,
            uint32(block.timestamp % 2 ** 32)
        );
        time = block.timestamp;
    ...
    function _updateAccumulators(
        ...
        if (totalFlow0 > 0 && totalFlow1 > 0) {
            ...
            if (flowDirection > totalFlow1) {
                ...
                uint112 swappedAmount1 = ((totalFlow1 *
                    timeElapsed *
                    (10000 - LOWER_BOUND_FEE)) / 10000) +
                    reserve1 -
                    _reserve1;
                twap0CumulativeLast += uint256(
                    UQ112x112.encode(swappedAmount0).uqdiv(totalFlow1)
                );
                twap1CumulativeLast += uint256(
                    UQ112x112.encode(swappedAmount1).uqdiv(totalFlow0)
                );
                ...
                uint112 swappedAmount0 = ((totalFlow0 *
                    timeElapsed *
                    (10000 - LOWER_BOUND_FEE)) / 10000) +
                    reserve0 -
                    _reserve0;
    ...
    function swap(
        ...
        uint256 amount1In;
        {
            ...
            uint112 _reserve1;
            {
                ...
                balance0 =
                    IERC20(_token0).balanceOf(address(this)) -
                    totalSwappedFunds0; // subtract locked funds that are not part of the reserves
                balance1 =
                    IERC20(_token1).balanceOf(address(this)) -
                    totalSwappedFunds1;
            }
    ...
    function _handleCallback(
        ...
        (uint112 totalFlow0, uint112 totalFlow1, int96 flow0, int96 flow1) = abi
            .decode(_cbdata, (uint112, uint112, int96, int96));
        ...
        if (address(_superToken) == _token0) {
            uint256 balance1 = UQ112x112.decode(
                uint256(uint96(flow0)) *
                    (twap1CumulativeLast - userStartingCumulatives1[user])
            );
            ...
            uint256 balance0 = UQ112x112.decode(
                uint256(uint96(flow1)) *
                    (twap0CumulativeLast - userStartingCumulatives0[user])
            );
        ...
        uint256 poolBalance0 = IERC20(_token0).balanceOf(address(this)) -
            totalSwappedFunds0; // subtract locked funds that are not part of the reserves
        uint256 poolBalance1 = IERC20(_token1).balanceOf(address(this)) -
            totalSwappedFunds1;
